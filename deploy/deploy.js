...
async function set_BDPMasterForBDPToken() {
  ...
  if (BDP_MASTER_ADDRESS == ADDRESS_0) throw "NO BDP_MASTER_ADDRESS"
  if (masterInBdpToken == ADDRESS_0) {
    console.log('CALL: Set BDPMaster for BDPToken')
    var tx = await BDP_TOKEN.setMaster(BDP_MASTER_ADDRESS, txParams(
      RPC,
      OWNER_ADDRESS,
      GAS_PRICE_TX,
      GAS_LIMIT
...
async function set_bAlphaMasterForbAlphaToken() {
  ...
  if (BALPHA_MASTER_ADDRESS == ADDRESS_0) throw "NO BALPHA_MASTER_ADDRESS"
  if (masterInbAlphaToken == ADDRESS_0) {
    console.log('CALL: Set bAlphaMaster for bAlphaToken')
    var tx = await BALPHA_TOKEN.setMaster(BALPHA_MASTER_ADDRESS, txParams(
      RPC,
      OWNER_ADDRESS,
      GAS_PRICE_TX,
      GAS_LIMIT
...
async function createPair_BDP_ETH() {
  ...
  if (BDP_ETH_PAIR == ADDRESS_0) {
    await uniswap.createPair(BDP_TOKEN_ADDRESS, WETH_ADDRESS, await txParams(
      RPC,
      OWNER_ADDRESS,
      GAS_PRICE_TX,
      GAS_LIMIT
...
async function createPair_BALPHA_ETH() {
  ...
  if (BALPHA_ETH_PAIR == ADDRESS_0) {
    await uniswap.createPair(BALPHA_TOKEN_ADDRESS, WETH_ADDRESS, await txParams(
      RPC,
      OWNER_ADDRESS,
      GAS_PRICE_TX,
      GAS_LIMIT
...
async function add_BDP_ETH_ToPoolInBAlphaMaster() {
  ...
  if (id == 0) {
    BDP_ETH_PAIR = await uniswap.getPair(BDP_TOKEN_ADDRESS, WETH_ADDRESS)
    if (BDP_ETH_PAIR == ADDRESS_0) throw "NO BDP_ETH_PAIR"
...
async function add_BALPHA_ETH_ToPoolInBAlphaMaster() {
  ...
  if (id == 0) {
    BALPHA_ETH_PAIR = await uniswap.getPair(BALPHA_TOKEN_ADDRESS, WETH_ADDRESS)
    if (BALPHA_ETH_PAIR == ADDRESS_0) throw "NO BALPHA_ETH_PAIR"
